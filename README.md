...
Build steps for Unix platform:

1. create a new directory for temporary file
   $ mkdir build && cd build

2. standard cmake configure & generate
   $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release ..

   or you want to build with other package (may contains other demos)

   There are 4 different kinds of cmake cinfigure mode in total:
   
           1 : The default, which only contains "skeleton_example"(cpp) 

                   $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release ..

           2 : The default plus the python version of "skeleton_example",
               which is named after "skeletonExampleOpenGL.py" (in the folder "lipsbodypose_3D_demo_python\ ") 

                   $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release -DENABLE_PYTHON=ON ..

           3 : The default plus "skeleton_example_boundingBox"(cpp)

                   $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release -DENABLE_VF_SAMPLE=ON ..

           4 : Contains all samples.

                   $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release -DENABLE_PYTHON=ON -DENABLE_VF_SAMPLE=ON ..

3. We also have to specify which kinds of camera device we are using for the prpogram. 
   For example, If we choose to compile all samples , then we will have two options, to use D415 or to use AE400.

   D415 :
       $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release -DENABLE_PYTHON=ON -DENABLE_VF_SAMPLE=ON -DCAM_RS_LIB=D415 ..

   AE400 :
       $ cmake -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=Release -DENABLE_PYTHON=ON -DENABLE_VF_SAMPLE=ON -DCAM_RS_LIB=AE400 ..

   Its the same for other modes as well.

4. If there is no any cmake errors reported, start compilation
   $ make

5. install OR do packaging
   $ make install # install files into 
   $ make package # generate tar.gz file in current directory

6. (OPTIONAL) license-gen tool
   $ make license-gen
   $ make install_tool
